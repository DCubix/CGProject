...
static const std::string K_HEADER = R"(
...
static const std::string K_IMAGE = R"(
    __kernel void ImageNode(__read_only image2d_t in, __write_only image2d_t out) {
		...
		write_imagef(out, ipos, read_imagef(in, sampler, uv));
    }
)";

static const std::string K_MUL = R"(
	__kernel void MulNode(__read_only image2d_t a, __read_only image2d_t b, __write_only image2d_t out) {
		const int2 ipos = (int2)(get_global_id(0), get_global_id(1));
        const float2 pos = (float2)((float) get_global_id(0), (float) get_global_id(1));
        const float2 sz = convert_float2(get_image_dim(out));
        const float2 uv = (float2)(pos.x / sz.x, pos.y / sz.y);
		...
		write_imagef(out, ipos, mres);
    }
)";

static const std::string K_ADD = R"(
	__kernel void AddNode(__read_only image2d_t a, __read_only image2d_t b, __write_only image2d_t out) {
		const int2 ipos = (int2)(get_global_id(0), get_global_id(1));
        const float2 pos = (float2)((float) get_global_id(0), (float) get_global_id(1));
        const float2 sz = convert_float2(get_image_dim(out));
        const float2 uv = (float2)(pos.x / sz.x, pos.y / sz.y);
